 ...
 */
package my.org.jitsi.impl.neomedia.codec.video.vp8;
    ...
    extends AbstractCodec2
{
    ...
    protected int doProcess(Buffer inBuffer, Buffer outBuffer)
    {
        ...
        if (empty && inMarker && inIsStartOfFrame)
        {
            ...
            try{ //GIANNIS_EDITED
	            System.arraycopy(
	                    ...
	                    0,
	                    inPayloadLength);
            }catch (ArrayIndexOutOfBoundsException | NegativeArraySizeException e){//GIANNIS_EDITED
            	System.err.println(e.getMessage());
            	System.err.println("inData:"+inData.length+" "+inOffset+" "+inPdSize+" "+inPayloadLength+" outData:"+outData.length);
            }
        ...
        //GIANNIS-EDITED
        if (inPayloadLength<0){
        	if (logger.isInfoEnabled()){
                logger.info("GIANNIS-EDITED:Negative payload length");
        	}
        	return BUFFER_PROCESSED_OK;
        }else if(container.buf == null || container.buf.length < inPayloadLength)
